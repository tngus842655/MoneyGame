// 런처 아이콘·스플래시·스토어 아이콘을 원본 아이콘 하나에서 만들어 낸다.
...
const SOURCE = 'brand/web-app-manifest-512x512.png'
const RES = 'android/app/src/main/res'
const STORE = 'store'

// 아트워크 가장자리에서 뽑은 파랑(#0056fd~#1b5caf의 중간). 적응형 아이콘 배경으로 깔면
// 원/스퀘어 어느 마스크로 잘려도 아트워크와의 이음매가 눈에 띄지 않는다.
const ICON_BG = '#0b63d6'
// index.html #splash / capacitor.config.js backgroundColor와 같은 값.
// 네이티브 스플래시 → 웹 스플래시로 넘어갈 때 색이 튀지 않게 맞춘다.
const SPLASH_BG = '#d4edfb'

const DENSITIES = { mdpi: 1, hdpi: 1.5, xhdpi: 2, xxhdpi: 3, xxxhdpi: 4 }

// 적응형 아이콘은 108dp 캔버스 중 안쪽 72dp만 어떤 마스크에서도 살아남는다.
// 아트워크가 가장자리까지 꽉 찬 그림이고 아래쪽에 '머니게임' 제목이 있어서,
// 72dp에 딱 맞추면 원형 마스크가 제목의 좌우를 잘라 먹는다.
// 68dp로 살짝 줄이고 3dp 위로 올리면 원형·둥근사각 양쪽에서 제목이 온전히 보인다.
const ART_DP = 68
const ART_SHIFT_DP = -3
...
async function launcherIcons() {
  for (const [density, scale] of Object.entries(DENSITIES)) {
    ...
    // API 25 이하가 쓰는 아이콘. 마스크가 없어 런처가 파일 그대로 보여주므로
    // 모서리를 미리 깎아 둔다.
...
`,
  ...
  // 기본 템플릿이 남긴 Capacitor 로고 벡터. 적응형 아이콘이 @color/@mipmap을 쓰므로
  // 아무 데서도 참조되지 않는다 (참조처는 mipmap-anydpi-v26/*.xml 두 개뿐).
  await rm(`${RES}/drawable/ic_launcher_background.xml`, { force: true })
  await rm(`${RES}/drawable-v24`, { recursive: true, force: true })   // 안에 그 벡터 하나뿐이었다
}
...
async function storeIcon() {
  ...
  // Play 스토어 아이콘 규격이 512×512 32비트 PNG라 원본을 그대로 쓴다.
  // 모서리는 Play가 알아서 깎으므로 각진 정사각형으로 둔다.
