...
//
// --toss: 앱인토스 번들용 — 구글플레이 전용 파일(AdMob 광고·플레이 결제)을 제외한다.
...
if (toss) {
  const PLAY_ONLY = ['js/ads-config.js', 'js/admob-ads.js', 'js/play-iap.js'];
  for (const p of PLAY_ONLY) rmSync(`dist/${p}`, { force: true });
  let html = readFileSync('dist/index.html', 'utf8');
  for (const p of PLAY_ONLY) {
    html = html.replace(new RegExp(`[ \\t]*<script src="${p}"></script>\\r?\\n`), '');
  }
  writeFileSync('dist/index.html', html);
  console.log('dist/ 준비 완료 (토스용 — 구글플레이 전용 파일 제외):', FILES.join(', '));
} else {
