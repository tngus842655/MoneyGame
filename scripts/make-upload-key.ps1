<#
.SYNOPSIS
플레이스토어 업로드 키를 만들고 android/keystore.properties까지 채운다.
...
⚠️ 만들어진 .keystore를 잃어버리면 com.moneygame.app은 영영 업데이트할 수 없다.
...
#>
param(
    # 기본값은 android/moneygame-upload.keystore. 테스트할 때만 바꾼다.
    [string] $KeystorePath,
    [string] $PropertiesPath,
    [string] $Alias = 'moneygame',
...
if (-not $KeystorePath)   { $KeystorePath   = Join-Path $root 'android\moneygame-upload.keystore' }
...
& $keytool -genkeypair -v `
    ...
    -storepass $Password `
    -keypass   $Password `
    -dname     'CN=Money Game, OU=Individual, O=Money Game, L=Seoul, ST=Seoul, C=KR'

if ($LASTEXITCODE -ne 0) { throw "키 생성 실패 (exit $LASTEXITCODE)" }

# gradle이 읽는 형식. storeFile은 android/ 기준 상대 경로.
$props = @(
    '# scripts/make-upload-key.ps1이 만든 파일. 커밋 금지 (.gitignore에 있음).'
