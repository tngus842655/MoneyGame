...
-- (몇 번이고 재실행해도 안전. 함수 하나만 고쳤을 땐 해당 블록만 실행해도 됨)
--
-- js/ranking.js는 이 파일의 rpc들로만 저장/조회하므로 마이그레이션이 필수입니다.
-- rpc가 없으면 랭킹 저장·조회만 실패하고 게임 플레이 자체는 정상 동작합니다.
--
-- 설계 메모:
...
--    비현실적으로 큰 점수는 조작으로 보고 조용히 버린다. 상한 10억 원:
--    실플레이에서 1000만 원대가 무리 없이 나와서(잭팟 반복), 상위권이 1억에
--    닿을 가능성을 보고 1억 → 10억으로 올렸다(2026-08-22). 정상 플레이로는
--    여전히 닿기 어려운 값이다 (클라이언트 신고형 랭킹의 최소한의 방어선).
