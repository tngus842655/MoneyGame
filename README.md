...
- 게임 내 **최고 기록 기준은 서버가 진실의 원천** — 시작 시 `get_my_best`(내
  버킷들의 최댓값)를 조회해 동기화하며, 응답이 오면 로컬 캐시보다 우선함
  (DB에서 기록을 지우면 낮아지는 것도 그대로 반영). localStorage 값은
  응답 전/오프라인 표시용 캐시로, 조회 실패 시에만 유지됨
