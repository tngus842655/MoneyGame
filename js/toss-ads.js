...
// ---------------------------------------------------------------- 광고 제거 구매자·관리자
// 인앱결제로 "광고 제거"를 산 기기와 관리자 기기(js/admin-mode.js — 무효 트래픽 방지)에서는
// 배너를 아예 안 붙이고 보상형도 미리 받지 않는다.
// (토스 SDK에 배너 해제 API가 없어서, 이미 붙은 뒤 상태가 바뀌면 내용을 비우고 슬롯을 숨긴다.)
const adFree = () =>
...
// 게임 도중에 구매하거나 관리자 판별(서버 답, 첫 실행)이 도착하면 그 자리에서 배너를 걷는다
